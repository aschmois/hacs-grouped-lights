...
describe('buildTree', () => {
  it('builds the nested area -> lamp -> bulb tree', () => {
    const root = buildTree(hass(OFFICE), 'light.room_lamps')!;
  ...
  it('returns null for a missing root entity', () => {
    expect(buildTree(hass(OFFICE), 'light.nope')).toBeNull();
  });
  ...
  it('memberIds/friendlyName helpers', () => {
    expect(memberIds(OFFICE['light.floor_lamp'])).toEqual(['light.bulb_1', 'light.bulb_2']);
    expect(memberIds(OFFICE['light.bulb_1'])).toEqual([]);
    expect(friendlyName(OFFICE['light.bulb_2'])).toBe('Bulb 2');
